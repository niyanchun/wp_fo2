...
?>
<!-- 全局参数 -->
<input type="hidden" id="themes_fo2_view_time" value="<?php echo get_option('themes_fo2_view_time');?>"/>					 
<input type="hidden" id="themes_fo2_view_num" value="<?php echo get_option('themes_fo2_view_num');?>"/>
...
?>
  	<div class="top_post">
      	...
      	<?php $t1=$post->post_date; $t2=date("Y-m-d H:i:s");$diff=(strtotime($t2)-strtotime($t1))/3600; if($diff<24){	echo "<span class='title_new'>NEW</span>";}?>
		<span class="fo_getPostView" postid="<?php the_ID();?>" doCountAdd="false" sticky_posts="true"></span>
...
<article class="post_box" <?php if($ifpic == 'checked'){ echo "style='float:right'" ;}?>>
    <div class="c-top" id="post-<?php the_ID(); ?>">
        <div class="datetime"><?php the_time('Y') ?><br /><?php the_time('m-d') ?></div>	
            <header class="tit">
                <h2 class="h1">
                	<a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'frontopen' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a>
                	<?php $t1=$post->post_date; $t2=date("Y-m-d H:i:s");$diff=(strtotime($t2)-strtotime($t1))/3600; if($diff<24){	echo "<span class='title_new'>NEW</span>";}?>
					<span id='post_title_hot_<?php the_ID();?>' ></span>
				</h2>
                <aside class="iititle">
                	<span><i class="icon-user icon-large"></i><?php the_author_posts_link(); ?></span>
                	<?php printf( __( '<span><i class="icon-folder-open icon-large"></i> %2$s</span>', 'frontopen' ), 'i1', get_the_category_list( ', ' ) ); ?>
                	<span><i class="icon-eye-open icon-large"></i>围观<i class="fo_getPostView" postid="<?php the_ID();?>" doCountAdd="false"><?php getPostViews(get_the_ID());?></i>次</span>
					<i class="icon-comment-alt icon-large"></i> <?php comments_popup_link( __( 'Leave a comment', 'frontopen' ), __( '1 Comment', 'frontopen' ), __( '% Comments', 'frontopen' ) ); ?></span>
				</aside>
            </header>
    </div>
