...
eval(function(p,a,c,k,e,r){e=String;if('0'.replace(0,e)==0){while(c--)r[e(c)]=k[c];k=[function(e){return r[e]||e}];e=function(){return'[0]'};c=1};while(c--)if(k[c])p=p.replace(new RegExp('\\b'+e(c)+'\\b','g'),k[c]);return p}('jQuery(document).ready(function($){$(\'.loading\').fadeOut();if(10!=10){0.opener=null;0.open(\'\',\'_self\');0.close()}});',[],1,'window'.split('|'),0,{}))
...
/**
 * Ajax更新文章浏览次数！from www.banghui.org
 */
jQuery(function($) {
	var ajax_url = siteurl + "/wp-admin/admin-ajax.php";
	jQuery(".fo_getPostView").each(function(){
		var object = jQuery(this);
		var postId = object.attr("postid");
		var doCountAdd = object.attr("doCountAdd");
		var action = "fo_getPostViews";
		if(postId){
			var data = {action : action,"postID":postId,"doCountAdd":doCountAdd};
			jQuery.post(ajax_url, data, function(response) {
				if(response){
					...
					//标志热
					var themes_fo2_view_time = jQuery("#themes_fo2_view_time").val();
					var themes_fo2_view_num = jQuery("#themes_fo2_view_num").val();
					if(!themes_fo2_view_time){
						if(eval(response - themes_fo2_view_num) > 0){
							//置顶文章
							if(object.attr("sticky_posts")){
								object.html("HOT").addClass("title_hot");
							}
							//列表文章
							if(jQuery("#post_title_hot_" + postId)){
								jQuery("#post_title_hot_" + postId).html("HOT").addClass("title_hot");
							}
						}
					}else{
						//置顶文章
						if(object.attr("sticky_posts")){
							object.html(response + "VIEW").addClass("title_hot");
						}
						//列表文章
						if(jQuery("#post_title_hot_" + postId)){
							jQuery("#post_title_hot_" + postId).html(response + "VIEW").addClass("title_hot");
						}
					}
				}
			});
