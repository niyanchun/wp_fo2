...
//2015-4-15 由于服务器接口无法抗压，暂时禁用 <开始>
//仪表盘首页frontopen相关消息面板
...
function dm_strimwidth($str ,$start , $width ,$trimmarker ){
    ...
    //短代码处理  by:晨风
    //正则替换
    $pattern = '/\[fo_hide\]([\s\S]*?)\[\/fo_hide\]/';
    $output = preg_replace($pattern,'<span style="color:red;">【内含福利】</span>',$output);
...
function post_thumbnail( $width,$height,$dataSrc){
	...
	$qiniuapi = "?imageView2/1/w/$width/h/$height";
 ...
 */
//针对未登录
add_action( 'wp_ajax_nopriv_fo_getPostViews', 'fo_getPostViews' );
//针对登录用户
add_action( 'wp_ajax_fo_getPostViews', 'fo_getPostViews' );
function fo_getPostViews(){
	...
	if($post_ID) 
	{
	  $focount = (int)get_post_meta($post_ID, $count_key, true);
	  $other = get_post_meta($postID, 'views', true);
	  $post_views = max($focount,$other) + 1;
	  if(isset($_POST['doCountAdd']) && $_POST['doCountAdd'] == "true"){
		  if(!update_post_meta($post_ID, $count_key, $post_views))
		  {
			add_post_meta($post_ID, $count_key, 1, true);
		  }
	  }
	}
	echo get_post_meta($post_ID, $count_key, true);;
	wp_die();
}
...
function my_custom_login_logo() {
	$logoUrl = str_replace(get_bloginfo('url'),'',get_option('themes_fo2_logo_img'));
    echo '<style type="text/css">
        h1 a { background-image:url('.get_bloginfo('url').''.$logoUrl.') !important; background-size:auto 84px !important; width:auto !important; height:84px !important;}
    </style>';
}
